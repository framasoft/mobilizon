        ...
        <div class="header flex flex-col">
          <div class="flex self-center h-0 mt-4 items-end">
            <figure class="" v-if="group.avatar">
              <img :src="group.avatar.url" alt="" width="128" height="128" />
            </figure>
    ...
    <div v-else-if="group" class="public-container flex flex-col">
        ...
        <div class="sticky">
          ...
          <event-metadata-block
            v-if="
              physicalAddress &&
              physicalAddress.url &&
              addressFullName(physicalAddress)
            "
            :title="t('Location')"
          >
            <template #icon>
      ...
      <div class="main-content min-w-min flex-auto py-0 px-2">
        <section>
          ...
          <div class="main-event-list">
            ...
            >
              <close-content
                ...
                v-show="
                  groupLoading || (group && organizedEvents.elements.length > 0)
                "
                v-on="attrs"
              >
                <template #content>
          ...
          <h2 class="text-xl font-bold subheading">{{ t("Aktion") }}</h2>
          <div class="main-event-list">
            <div
              class="event-list flex flex-col gap-3 w-[960px]"
            ...
            >
              <close-content
                class="container mx-auto px-2"
                v-show="
                  groupLoading || (group && EventsMeeting.elements.length > 0)
                "
                v-on="attrs"
              >
                <template #content>
...
const EventsMeeting = computed((): Paginate<IEvent> => {
  return {
    total:
      group.value?.organizedEvents.elements.filter(
        (event: IEvent) => event.category === "MEETING"
      ).length ?? 0,
    elements:
      group.value?.organizedEvents.elements.filter((event: IEvent) => {
        if (previewPublic.value) {
