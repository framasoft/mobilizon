<template>
  ...
  <div class="flex-1 m-0 flex flex-col flex-wrap gap-2">
    <!-- Text field to display group summary -->
    <div v-if="summaryResult && summaryResult.data">
      <textarea v-model="groupSummary" class="w-full h-40" readonly></textarea>
    </div>
  </div>
...
const preferredUsername = route.query.g;
...
const { result: groupPostsResult, loading: groupLoading } = useQuery<{
  ...
  () => ({ enabled: preferredUsername !== undefined })
);

const { result: summaryResult, loading: summaryLoading } = useQuery<{
}>(
  FETCH_GROUP_PUBLIC,
  () => ({
...
const groupSummary = computed(() => {
  if (summaryResult.value && summaryResult.value.data) {
    const summaryData = summaryResult.value.data;
    // Assuming your summary is stored in a field named "summary"
